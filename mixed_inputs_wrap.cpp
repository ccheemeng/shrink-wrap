...
{
  // Read the inputs
  const std::string ts_filename = (argc > 1) ? argv[1] : CGAL::data_file_path("data/meshes/armadillo.off"); // triangle soup
  const std::string ss_filename = (argc > 2) ? argv[2] : CGAL::data_file_path("data/images/420.polylines.txt"); // segment soup
  const std::string ps_filename = (argc > 3) ? argv[3] : CGAL::data_file_path("data/points_3/ball.ply"); // point set
